    ...
    return Scaffold( // 画面の基本的なレイアウトを提供
      appBar: AppBar( // アプリバーを定義
        leading: IconButton( // 戻るボタンを定義
          icon: const Icon(Icons.arrow_back), // 閉じるアイコンを戻るアイコンに変更
          onPressed: () { // ボタンが押されたときの処理
            Navigator.of(context).pop(); // 一覧画面に戻る
          },
        ),
        ...
        actions: [ // アプリバーのアクションボタン
          TextButton(
            onPressed: () { // 保存ボタンが押されたときの処理
              // TODO: 後で保存機能を実装
              Navigator.of(context).pop(); // テスト用：一覧画面に戻る
            },
            child: const Text(
              '保存', // ボタンのテキスト
      ...
      body: Padding( // パディングを追加
        ...
        child: Column( // 縦にウィジェットを並べる
          ...
          children: [
            ...
            const SizedBox(height: 16.0), // ボタンの上にスペースを追加
            Row(
